...
namespace {
	...
	struct EO_Display : TransparentWidget {
		...
		void draw(const DrawArgs &args) override {
			...
			for (int i = 0; i < 2; i++) {
				if (module->inputs[HS_101::INPUT_1 + i].isConnected()) {
			...
			drawIndex(args.vg, clamp(module->params[HS_101::PARAM_INDEX_1].getValue(), 0.0f, 1.0f));
			drawIndex(args.vg, clamp(module->params[HS_101::PARAM_INDEX_2].getValue(), 0.0f, 1.0f));
			drawIndexV(args.vg, clamp(module->params[HS_101::PARAM_INDEX_3].getValue(), 0.0f, 1.0f));
			if (module->inputs[HS_101::INPUT_EXT].isConnected())
				drawTrigger(args.vg, module->params[HS_101::PARAM_TRIGGER].getValue(), 0.0f, 1.0f);
			else
				drawTrigger(args.vg, module->params[HS_101::PARAM_TRIGGER].getValue(), module->params[HS_101::PARAM_OFFSET_1].getValue(), module->params[HS_101::PARAM_SCALE_1].getValue());
			drawMask(args.vg, clamp(module->params[HS_101::PARAM_PRE].getValue(), 0.0f, 1.0f * PRE_SIZE) / BUFFER_SIZE);
	...
	struct EO_Measure_Vert : EO_Measure {
		...
		void updateText() override {
			...
			float height = ((module->params[HS_101::PARAM_INDEX_3].getValue() - 0.2f) * 20.0f - module->params[HS_101::PARAM_OFFSET_1 + index].getValue()) / powf(2, module->params[HS_101::PARAM_SCALE_1 + index].getValue());
...
struct HS101 : SchemeModuleWidget {
	...
	HS101(HS_101 *module) {
		...
		for (int i = 0; i < 2; i++) {
			PortWidget *port = createInputCentered<SilverPort>(Vec(16.5 + 75 * i, 326.5), module, HS_101::INPUT_1 + i);
			addInput(port);
			display->ports[i] = port;
			addParam(createParamCentered<SubSwitch2>(Vec(16.5 + 75 * i, 280), module, HS_101::PARAM_MODE_1 + i));
			addParam(createParamCentered<MedKnob<LightKnob>>(Vec(50 + 75 * i, 320), module, HS_101::PARAM_OFFSET_1 + i));
			addParam(createParamCentered<SnapKnob<MedKnob<LightKnob>>>(Vec(50 + 75 * i, 270), module, HS_101::PARAM_SCALE_1 + i));
		}
		addParam(createParamCentered<MedKnob<LightKnob>>(Vec(172.5, 320), module, HS_101::PARAM_TIME));
		addParam(createParamCentered<SnapKnob<MedKnob<LightKnob>>>(Vec(172.5, 270), module, HS_101::PARAM_PRE));

		addInput(createInputCentered<SilverPort>(Vec(211.5, 326.5), module, HS_101::INPUT_EXT));
		addParam(createParamCentered<MedKnob<LightKnob>>(Vec(245, 320), module, HS_101::PARAM_TRIGGER));
		addChild(createLightCentered<TinyLight<BlueLight>>(Vec(226, 333), module, HS_101::LIGHT_TRIGGER));
		addParam(createParamCentered<SubSwitch2>(Vec(211.5, 280), module, HS_101::PARAM_RUNMODE));
		paramRun = createParamCentered<LightButton>(Vec(245, 280), module, HS_101::PARAM_RUN);
		addParam(paramRun);

		addParam(createParamCentered<MedKnob<LightKnob>>(Vec(290, 320), module, HS_101::PARAM_INDEX_1));
		addParam(createParamCentered<MedKnob<LightKnob>>(Vec(332, 320), module, HS_101::PARAM_INDEX_2));
		addParam(createParamCentered<MedKnob<LightKnob>>(Vec(376, 320), module, HS_101::PARAM_INDEX_3));
	}
